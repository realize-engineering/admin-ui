...
const CreateDestination: NextPage = () => {
  ...
  const { data: destination, error } = useSWR<Destination>(
    `/destinations/${id}`,
    fetcher,
  )
  const {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      await api.patch(`/destinations/${id}`, data)
      Router.push("/")
    } catch (err) {
  ...
  useEffect(() => {
    if (destination) {
      // todo(ianedwards): try to fix type error causing reset to not compile here
      setValue("destinationType", destination.destinationType)
      setValue("nickname", destination.nickname)
      if (destination.destinationType === "SNOWFLAKE") {
        setValue("warehouse", destination.warehouse)
      }
    }
